						...
						<Flex bg={"black"}>
							<Stack spacing={4} w={"full"} maxW={"md"} bg={"black"} p={6} my={0}>
								...
								<FormControl>
									...
									<Input placeholder={"Username"} size={"sm"} type={"text"} value={inputs.name || user.username} onChange={(e)=>setInputs({...inputs,name:e.target.value})}/>
								</FormControl>

								<FormControl>
									<FormLabel fontSize={"sm"}>Bio</FormLabel>
									<Input placeholder={"Bio"} size={"sm"} type={"text"} value={inputs.bio || user.bio} onChange={(e)=>setInputs({...inputs,bio:e.target.value})}/>
								</FormControl>
